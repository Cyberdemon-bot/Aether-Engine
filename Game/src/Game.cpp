...
void Game::Attach()
{
    Aether::Application::Get().ToggleConsole(false);
...
Aether::Entity Game::SpawnZombie(const glm::vec3& position)
{
...
float Game::GetCellValue(int coordX, int coordZ) const
{
...
int Game::GetObstacleCost(int coordX, int coordZ) const
{
    return (GetCellValue(coordX, coordZ) > 0.0f) ? 255 : 1;
}

bool Game::IsObstacle(const glm::vec3& worldPos) const
{
...
bool Game::IsObstacleWithRadius(const glm::vec3& worldPos) const
{
...
float Game::GetSpeedMultiplier(const glm::vec3& worldPos) const
{
    const float r = k_CapsuleRadius + k_CollisionSkin;
...
void Game::UpdateFlowField(const glm::vec3& targetPos)
{
...
void Game::OnEvent(Aether::Event& event)
{
